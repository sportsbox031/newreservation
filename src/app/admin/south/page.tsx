...
export default function AdminSouthDashboard() {
  ...
  const checkAuth = () => {
    ...
    if (adminData.role !== 'south' && adminData.role !== 'super') {
      alert('접근 권한이 없습니다.')
      router.push('/auth/login')
      return
    }
  ...
  const loadDashboardData = async (adminData: any) => {
    try {
      ...
      const [membersResult, reservationsResult, announcementsResult] = await Promise.all([
        memberAPI.getPendingMembersForRegion('south'),
        reservationAPI.getPendingReservationsForRegion('south'),
        announcementAPI.getPublicAnnouncements()
      ])

      // 승인된 회원 수 조회
      const approvedMembersResult = await memberAPI.getApprovedMembersForRegion('south')
      const approvedReservationsResult = await reservationAPI.getApprovedReservationsForRegion('south')
      ...
      // 최근 활동 데이터 구성 (경기남부 지역만)
      const activities: RecentActivity[] = [
        ...(membersResult.data || []).slice(0, 3).map((member: any) => ({
          id: member.id,
          type: 'member' as const,
          title: `${member.organization_name} 회원가입 신청 (경기남부)`,
        ...
        ...(reservationsResult.data || []).slice(0, 3).map((reservation: any) => ({
          ...
          title: `${reservation.users?.organization_name} 예약 신청 (${reservation.date}) - 경기남부`,
      ...
      <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">최근 활동 - 경기남부</h2>
          </div>
