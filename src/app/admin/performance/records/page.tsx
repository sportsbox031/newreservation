...
import { Plus, Edit2, Trash2, Download, Search, RotateCcw } from 'lucide-react'
...
import type { PerformanceRecord, PerformanceProgram } from '@/lib/performanceTypes'
...
function AdminPerformanceRecordsContent() {
  ...
  const handleAddExperience = async () => {
    ...
    try {
      ...
      const json = await readJsonSafely(res)
      if (!res.ok) {
        alert(json?.error?.message || '체험존 실적 저장에 실패했습니다.')
        return
      }
  ...
  const handleSaveExperienceEdit = async () => {
    ...
    try {
      ...
      const json = await readJsonSafely(res)
      if (!res.ok) {
        alert(json?.error?.message || '체험존 실적 수정에 실패했습니다.')
        return
      }
      setEditing(null)
      await loadRecords()
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-900">실적 상세 조회</h1>
      ...
      {showAddModal && (
        <ModalOverlay onClose={() => setShowAddModal(false)} closeOnBackdrop={false}>
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">체험존 실적 추가</h2>
...
export default function AdminPerformanceRecordsPage() {
  return (
    <Suspense fallback={
      ...
      <AdminPerformanceRecordsContent />
    </Suspense>
  )
}
