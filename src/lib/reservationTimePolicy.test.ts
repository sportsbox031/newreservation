...
test('reservation start time options begin at 10:00', () => {
  ...
  assert.equal(options.includes('09:50'), false)
  assert.equal(options.includes('10:00'), true)
...
test('reservation start time policy rejects times outside the allowed range', () => {
  ...
  assert.equal(isReservationStartTimeAllowed('09:50'), false)
  assert.equal(isReservationStartTimeAllowed('10:00'), true)
