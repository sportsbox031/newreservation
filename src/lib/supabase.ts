...
const hashPassword = (password: string): string => {
  ...
  return btoa(password + 'sportsbox_salt')
}
...
export const settingsAPI = {
  ...
  async getDateReservationStatus(regionCode: string, date: string) {
    ...
    maxReservationsPerDay = 2 // 기본값
    isOpen = false // 예약 종료가 기본값
  ...
  async getMonthReservationStatus(regionCode: string, year: number, month: number) {
    ...
    for (let day = 1; day <= lastDay; day++) {
      ...
      // 1. 월별 예약시작/종료 설정이 최우선 (관리자 버튼)
      if (settings?.is_open === false) {
        // 관리자가 예약종료로 설정하면 모든 날짜 닫힘
        isOpen = false
      }
      else if (settings?.is_open === true) {
        // 관리자가 예약시작으로 설정하면 기본적으로 열림
        isOpen = maxReservations > 0
        
        // 2. 개별 날짜 설정으로 세부 조정 (예약불가, 갯수제한)
        if (dailyLimitMap[dateString]) {
          // 개별 설정이 0이면 해당 날짜만 예약불가
          isOpen = maxReservations > 0
        }
      }
      // 3. 월별 설정이 없으면 기본값(닫힘)
