-- 간소화된 예약 동시성 제어 함수 (시간대 제거, 용어 통일)
...
DROP FUNCTION IF EXISTS check_and_reserve(UUID, INTEGER, DATE, INTEGER);
DROP FUNCTION IF EXISTS check_user_monthly_limit(UUID, INTEGER, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS check_user_daily_limit(UUID, DATE, INTEGER);

-- 2. 하루 최대예약개수 체크 및 예약 가능 확인 함수
CREATE OR REPLACE FUNCTION check_daily_reservation_limit(
  p_user_id UUID,
  p_region_id INTEGER,
  p_date DATE,
...
BEGIN
  ...
  -- 1. 해당 날짜의 현재 예약 개수 조회 (FOR UPDATE로 락)
  SELECT COUNT(*)
  ...
  ELSE
    ...
    v_result := json_build_object(
      ...
      'current_count', v_current_count,
      'max_allowed', v_max_allowed,
      'available_count', v_max_allowed - v_current_count
    );
  END IF;
...
-- 완료 메시지
SELECT '간소화된 동시성 제어 함수가 생성되었습니다!' as status;